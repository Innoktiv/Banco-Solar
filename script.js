const getUsuarios = async () => {
    ...
    $.each(data, (i, c) => {
        ...
        $(".usuarios").append(`
            <tr>
                <td><span class="math-inline">\{c\.nombre\}</td\><td\></span>{c.balance}</td>
                <td>
                    <button class="btn btn-warning mr-2" data-toggle="modal" data-target="#exampleModal" onclick="setInfoModal('<span class="math-inline">\{c\.nombre\}', '</span>{c.balance}', '<span class="math-inline">\{c\.id\}'\)"\> Editar </button\>
                    <button class\="btn btn\-danger" onclick\="eliminarUsuario\('</span>{c.id}')">Eliminar</button>
                </td>
            </tr>
        `);

        $("#emisor").append(`<option value="<span class="math-inline">\{c\.nombre\}"\></span>{c.nombre}</option>`);
        $("#receptor").append(`<option value="<span class="math-inline">\{c\.nombre\}"\></span>{c.nombre}</option>`);
    });
};
